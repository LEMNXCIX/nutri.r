...
use crate::tauri_bridge::{calculate_nutrition, get_calendar_range, get_index, MealType};
use chrono::Local;
use leptos::prelude::*;
...
pub fn Home() -> impl IntoView {
    // Water Logic (Ephemeral for now)
    let (water_current, set_water_current) = signal(1.5f32);
    let water_target = 2.5f32;

    let on_add_water = Callback::new(move |_| {
        set_water_current.update(|v| *v = (*v + 0.25f32).min(5.0f32));
    });

    let on_remove_water = Callback::new(move |_| {
        set_water_current.update(|v| *v = (*v - 0.25f32).max(0.0f32));
    });
                                ...
                                if plans.is_empty() {
                                    view! {
                                        <div class="card p-8 flex flex-col items-center justify-center gap-3 h-[280px]">
                                            ...
                                            <a href="/generate" class="mt-4 px-4 py-2 bg-blue-500 text-white rounded-xl text-[10px] font-bold uppercase tracking-wider hover:bg-blue-600 transition-colors shadow-soft-blue">"CREAR PLAN"</a>
                                        </div>
                                    }.into_any()
                                } else {
