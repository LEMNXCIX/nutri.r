...
pub fn TagSelector(
    ...
    view! {
        <div class="relative inline-block">
            <button
                class="flex items-center gap-2 px-3 py-2 brutalist-border text-[10px] font-black uppercase tracking-wider bg-white text-neutral-400 hover:text-neutral-950 transition-all active:scale-95"
            ...
            {move || if show_dropdown.get() {
                view! {
                    <div class="absolute top-full left-0 mt-2 w-64 bg-white brutalist-border shadow-brutalist z-50 p-3 overflow-hidden">
                        <div class="relative space-y-3">
                            <div class="max-h-48 overflow-y-auto space-y-1 pr-1">
                                {move || {
                                    ...
                                    if tags.is_empty() {
                                        view! {
                                            <p class="text-[9px] font-bold uppercase tracking-widest text-neutral-400 text-center py-2">"No tags available"</p>
                                        }.into_any()
                                    } else {
                                        tags.into_iter().map(|tag| {
                                            ...
                                            view! {
                                                <button
                                                    class="w-full text-left px-3 py-2 hover:bg-neutral-50 transition-all flex items-center gap-2 group active:scale-95"
                                                    on:click=move |_| {
                                                ...
                                                >
                                                    ...
                                                    <span class="text-[10px] font-bold text-neutral-400 group-hover:text-neutral-950 uppercase tracking-wider">{tag.name}</span>
                                                </button>
                                            }
                            ...
                            <div class="pt-3 border-t border-neutral-100">
                                <div class="flex flex-col gap-2">
                                    <input
                                        type="text"
                                        placeholder="New tag..."
                                        class="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-neutral-950 outline-none focus:ring-1 focus:ring-accent"
